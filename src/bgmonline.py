...
class BackgammonClient:
    ...
    def sign(self, session_key: int):
        hash = b64encode(
            md5((f"{session_key}oc3q7ingf978mx457fgk4587fg847").encode()).digest()).decode()
    ...
    def create_game(
        ...
        if password:
            ...
            data = {"type": type, "bet": bet, "cube": cube, "fast": fast}
        self.send_server(data)

    def lookup_start(
        self, type: list = [
            1, 2], pr: bool = False, cube: list = [
            False, True], fast: list = [
                False, True], bet_min: int = 100, bet_max: int = 1000000, full: bool = False):
        self.send_server(
            {
    ...
    def send_smile_in_game(
            self,
            position: int = 1,
            smile_name: str = "smile_classic",
            smile_id: int = 7):
        self.send_server({"command": "smile", "p": position,
                         "id": smile_id, "a": smile_name})
    ...
    def buy_premium(self, id: int = 0):
        self.send_server(
            {"command": "buy_prem", "id": f"com.rstgames.bg.prem.{id}"})

    def buy_points(self, id: int = 0):
        self.send_server({"command": "buy_points",
                          "id": f"com.rstgames.bg.points.{id}"})
    ...
    def get_validate_rw(self):
        self.send_server({"command": "get_validate_rw"})

    def get_hall_gold(self):
        self.send_server({"command": "get_hall_gold"})
        return self.receive_server_response(unmarshal=True)

    def get_hall_silver(self):
        self.send_server({"command": "get_hall_silver"})
        return self.receive_server_response(unmarshal=True)

    def get_hall_bronze(self):
        self.send_server({"command": "get_hall_bronze"})
        return self.receive_server_response(unmarshal=True)
