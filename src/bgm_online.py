...
class BackgammonOnline:
	def __init__(
			self,
			platform: str = "android",
			debug: bool = False,
			language: str = "ru",
			tag: str = "backgammon_online".upper()) -> None:
		self.tag = tag
		self.user_id = None
		self.logger = logger
	...
	def buy_premium(self, id: int = 0) -> None:
		self.send_server({"command": "buy_prem", "id": f"com.rstgames.bg.prem.{id}"})

	def buy_points(self, id: int = 0) -> None:
		self.send_server(
			{"command": "buy_points", "id": f"com.rstgames.bg.points.{id}"})
